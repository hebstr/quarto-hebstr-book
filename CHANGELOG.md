...
- Reduced the rendered scale, which was oversized at 100% browser zoom: removed the `fontsize: 1.2rem` root override (root font size back to the default 16px) and narrowed the grid (`sidebar-width` 350px to 300px, `body-width` 1000px to 780px, `margin-width` 400px to 300px).

## [Unreleased]

### Added

- `hebstr-book` is now a standalone Quarto book theme extension, with no dependency on `hebstr-doc`.
